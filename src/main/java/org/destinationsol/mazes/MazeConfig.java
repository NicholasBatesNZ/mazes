...
public class MazeConfig {
    ...
    public static MazeConfig load(String module, String name, JSONObject mazeNode, HullConfigManager hullConfigs, ItemManager itemManager) {
        CollisionMeshLoader collisionMeshLoader = new CollisionMeshLoader(module + ":" + name + "Maze");
        CollisionMeshLoader.Model paths = collisionMeshLoader.getInternalModel();
        List<TextureAtlas.AtlasRegion> innerBackgrounds = Assets.listTexturesMatching("[a-zA-Z0-9]*:" + name + "MazeInnerBg_.*");
        List<TextureAtlas.AtlasRegion> borderBackgrounds = Assets.listTexturesMatching("[a-zA-Z0-9]*:" + name + "MazeBorderBg_.*");
        List<TextureAtlas.AtlasRegion> wallTextures = Assets.listTexturesMatching("[a-zA-Z0-9]*:" + name + "MazeWall_.*");
        List<TextureAtlas.AtlasRegion> passTextures = Assets.listTexturesMatching("[a-zA-Z0-9]*:" + name + "MazePass_.*");
